...
namespace DBLayerPOC.Infrastructure.SalesInvoice
{
    public class SalesInvoiceLineEntityTypeConfiguration : IEntityTypeConfiguration<SalesInvoiceLine>
    {
        public void Configure(EntityTypeBuilder<SalesInvoiceLine> builder)
        {
            ...
            builder.Property(x => x.LoadDate).HasColumnName("LoadDate").HasDefaultValue(DateTime.Now);
            builder.Property(x => x.UnloadDate).HasColumnName("UnloadDate").HasDefaultValue(DateTime.Now);
            builder.Property(x => x.LoadAddress).HasColumnName("LoadAddress").HasMaxLength(250).IsRequired(true);
            builder.Property(x => x.ExportCustoms).HasColumnName("ExportCustoms").HasMaxLength(250).IsRequired(true);
            builder.Property(x => x.ImportCustoms).HasColumnName("ImportCustoms").HasMaxLength(250).IsRequired(true);
        }
    }
}
